...
function log_message_text(str_Message, request) {
  ...
  var data = new Date(dt.now()) + request + str_Message;
  convert(data);
}
...
app.listen(
  ...
  /*IP,*/ () => {
    log_message_text(
      "INICIO MICROSERVICIO Usuarios - Corriendo en http://35.188.13.6:" + PORT,
      "780 SA -"
    );
  }
);
...
app.get("/jugadores", function (req, res) {
  ...
  log_message_text(
    " * SE TERMINO DE PROCESAR LA PETICION",
    "/jugadores -> GET"
  );
});
...
app.post("/jugadores", (req, res) => {
  ...
  log_message_text(
    " * Los datos recibidos fueron: " + JSON.stringify(req.body),
    "/jugadores -> POST"
  );

  if (
    req.body.nombres !== undefined &&
    req.body.apellidos != undefined &&
    req.body.email != undefined &&
    req.body.administrador !== undefined &&
    ...
    var sql =
      ...
      req.body.password +
      '", ' +
      (req.body.administrador ? 1 : 0) +
      ");";
    conn.query(sql, function (err, records) {
      if (err) {
        log_message_text(
          " * Error al registrar el usuaro " + JSON.stringify(err),
          "/jugadores -> POST"
        );
    ...
    log_message_text(
      " * Los datos recibidos son incorrectos",
      "/jugadores -> POST"
    );
...
app.put("/jugadores/:id", (req, res) => {
  ...
  if (
    ...
    conn.query(sql, function (err, records) {
      if (err) {
        log_message_text(
          " * Error al actualizar el usuaro " + JSON.stringify(err),
          "/jugadores/:id -> PUT"
        );
...
app.get("/jugadores/:id", (req, res) => {
  ...
  conn.query(sql, function (err, records) {
    if (err) {
      log_message_text(
        " * Error al obtener el usuaro " + JSON.stringify(err),
        "/jugadores/:id -> GET"
      );
      res.status(404).send(JSON.parse(JSON.stringify(err)));
      return;
    ...
    log_message_text(
      " * Usuario obtenido correctamente, con id: " + req.params.id,
      "/jugadores/:id -> GET"
    );
...
app.get("/login", (req, res) => {
  ...
  log_message_text(
    " * Los datos recibidos fueron: " + JSON.stringify(req.query),
    "/login -> GET"
  );
